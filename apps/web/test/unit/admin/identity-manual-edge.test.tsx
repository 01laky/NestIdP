...
import { IdentityMembershipPicker } from '@/admin/components/identity/IdentityMembershipPicker';
...
describe('Identity manual CRUD — web edge', () => {
	...
	it('WEB-ADM-CONF-07: delete user skipped when dialog cancelled (was WEB-IDN-MAN-13)', async () => {
		...
		const { clickDialogCancel } = await import('@test/helpers/confirm-dialog-helpers');
	...
	it('WEB-ADM-CONF-07b: delete user calls API when dialog confirmed (was WEB-IDN-MAN-14)', async () => {
		...
		await waitFor(() => screen.getByRole('button', { name: 'Delete' }));
		fireEvent.click(screen.getByRole('button', { name: 'Delete' }));
		const { clickDialogConfirm } = await import('@test/helpers/confirm-dialog-helpers');
		await screen.findByRole('dialog');
