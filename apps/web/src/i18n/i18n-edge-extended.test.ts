...
import { formatDateTime, getI18n, initI18n } from './i18n';

const i18nDir = import.meta.dirname;
...
describe('i18n edge — extended (WEB-I18N-79–95)', () => {
	...
	it('WEB-I18N-82: vitest setup-i18n forces en via initI18nForTests', () => {
		const setup = readFileSync(join(webSrc, 'test/setup-i18n.ts'), 'utf8');
		expect(setup).toContain("initI18nForTests('en')");
	});

	it('WEB-I18N-83: renderWithUi wraps I18nextProvider', () => {
		const render = readFileSync(join(webSrc, 'test/renderWithUi.tsx'), 'utf8');
