...
| POST   | `/api/admin/idp/settings/signing-cert/*`             | admin session                 | Generate/upload primary cert; rotation start/complete/cancel (CSRF). **Generate** accepts optional JSON: `keyFamily`, `rsaModulusBits`, `ecCurve`, `signatureAlgorithmId`, `notAfter` (`YYYY-MM-DD` UTC); `{}` = RSA-2048 / rsa-sha256 / 730d. Web imports options from `@nestidp/shared`. |
| POST   | `/api/admin/idp/settings/encryption-cert/generate`   | admin session + CSRF          | Generate IdP encryption cert: `keyFamily`, `rsaModulusBits`, `ecCurve`, `keyTransportAlgorithmId` (RSA only), `notAfter`; `{}` = RSA-2048 / rsa-oaep-mgf1p / 730d. No lazy generate on SSO.                                                                                                |
| POST   | `/api/admin/idp/settings/encryption-cert/upload`     | admin session + CSRF          | Upload encryption PEM pair; infers RSA/EC; default transport `rsa-oaep-mgf1p` for RSA. Rejects signing-only certs and duplicate of primary signing cert.                                                                                                                                   |
| POST   | `/api/admin/idp/settings/encryption-cert/rotation/*` | admin session + CSRF          | Encryption rotation start/complete/cancel (independent from signing rotation).                                                                                                                                                                                                             |
| GET    | `/api/admin/idp/settings/encryption-cert/public-pem` | admin session                 | Primary encryption certificate PEM only (no private key).                                                                                                                                                                                                                                  |
...
Operator UI: **`/admin/settings/idp`** (sidebar **IdP Settings**). Dashboard shows **`idp.certStatus`** (`missing` | `ok` | `expiring_soon` | `rotation_active`) and **`idp.encryptionCertStatus`** (`not_configured` | …) without a second API call.

Three certificate roles (signing, optional IdP encryption, per-SP cert): see [idp-certificates.svg](./img/idp-certificates.svg).
...
**Encryption certificate lifecycle (v1.5.0, independent):** same generate/upload/rotation pattern on `/encryption-cert/*` routes; no lazy generate on SSO; metadata adds `use="encryption"` descriptors when configured. Concurrent signing + encryption rotation is allowed.
...
| **Identity list tables**  | `IdentityListTable` (TanStack Table v8, `manualPagination`) + `useIdentityListQuery` (`IDENTITY_LIST_PAGE_SIZE` = 10, race-safe fetch, page clamp); pagination nav `.evg-table-pagination` with `aria-live` range            | Users / groups / roles browse pages                |
...
| `WEB-IDN-TBL-01`–`24`      | Identity list pages: pagination, filters, empty states, a11y (`identity-list-pagination.test.tsx`)                                                                                                        |
| `WEB-IDN-TBL-HK-01`–`10`   | `useIdentityListQuery` hook: loading/error/race/clamp/reload (`useIdentityListQuery.edge.test.tsx`)                                                                                                       |
| `WEB-IDN-TBL-CMP-01`–`11`  | `IdentityListTable`: bounds, disabled buttons, live region (`IdentityListTable.edge.test.tsx`)                                                                                                            |
...
| `WEB-I18N-116`             | Admin login Slovak remember checkbox labels (`i18n.integration.extended.test.tsx`)                                                                                                                        |
| `API-AUD-ADM-RM-01`–`06`   | Audit metadata `rememberMe` on login success/failure (`admin-auth.integration.spec.ts`, `admin-auth-audit.service.spec.ts`)                                                                               |
| `API-SES-16`–`24`          | `AdminSessionService` persistent vs session cookies, TTL cap, env fallback                                                                                                                                |
| `API-CTL-10`–`12`          | Admin auth controller rememberMe → TTL and persistent cookie (`admin-auth.controller.spec.ts`)                                                                                                            |
| `SH-ADM-07`                | Shared `AdminLoginRequestDto` optional `rememberMe` (`admin-auth.spec.ts`)                                                                                                                                |
| `API-ENV-ADM-RM-01`–`03`   | `ADMIN_SESSION_REMEMBER_TTL_SECONDS` validation                                                                                                                                                           |
| `API-CTL-10`–`11`          | Login controller passes TTL and `persistent` to `setCookie`                                                                                                                                               |
| `SH-ADM-04`–`06`           | Shared remember constants and storage key                                                                                                                                                                 |
| `CI-DCK-02-RM-01`          | Docker smoke: `rememberMe` login returns `Max-Age` (`scripts/ci-docker-smoke.sh`)                                                                                                                         |
| `WEB-IDN-TBL-E2E-01`       | Playwright users list Next + range text (`e2e/identity-list-pagination.spec.ts`)                                                                                                                          |
